...
本仓库收录 **Agent 行为偏差诊断 / 隐性失败诊断** 相关论文。重点不再只是“程序报错后反向定位原因”，而是关注更真实、更难发现的问题：
...
```text
.
├── 最相关/          # 核心必读：隐性失败、状态偏差、过程异常、失败归因
├── 高度相关/        # 方法可借鉴：层次归因、干预验证、结构化日志、过程奖励
├── 相关/            # 背景参考：产品形态、传统调试、可解释报告、自反思机制
└── scripts/          # 辅助脚本，例如新增论文 PDF 下载脚本
```

每个论文目录包含：

- `README.md` — 中文解读笔记
- `paper.pdf` — 论文原文；新增论文可运行 `scripts/download_new_papers.sh` 下载
...
新增论文的 README 已经放入仓库。由于 GitHub 文本写入接口不能直接上传 PDF 二进制文件，新增论文 PDF 可在本地运行：

```bash
bash scripts/download_new_papers.sh
```

脚本会把新增论文下载到对应目录的 `paper.pdf`。
