...
PROTEA 为多 Agent LLM 工作流提供离线评估和迭代改进框架。它最重要的点不是最终结果错了以后找报错，而是对中间节点输出进行评分，并从最终答案反推节点级期望。
...
你关心的问题是：某个 Agent 少写了 WorkState 字段，后续 Agent 仍然正常执行，流程没有报错，但最终产物交付不理想。

这不是传统编译错误，也不是工具调用异常，而是中间节点输出不满足期望。

PROTEA 的思想可以直接迁移成：Task 目标 -> 阶段级期望 -> WorkState / Artifact / Tool Evidence 检查 -> 中间节点评分 -> 定位第一个不满足期望的步骤。

## 可借鉴设计

### 1. Intermediate Output Scoring

对每个阶段产物打分：需求理解是否完整、设计是否覆盖关键约束、WorkState 是否包含必要字段、代码修改是否覆盖目标文件、验证命令是否真实执行。

### 2. Node-Level Expectation

为每个阶段生成“应该产出什么”，例如 planning 阶段需要明确目标、列出关键文件、定义验证方式；implementation 阶段需要修改目标文件、更新状态字段、保留 diff 证据；verification 阶段需要执行测试并记录结果。

### 3. Workflow Bottleneck View

在 AgentLens Viewer 中可以展示哪个阶段满足预期，哪个阶段出现 WorkState 缺字段，哪个后续阶段因此被跳过。
